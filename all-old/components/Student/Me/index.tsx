...
const StudentMe: React.FC = () => {
  ...
  return (
    <>
      <div className="border-b-slate-200 bg-slate-50 sticky top-0 z-10 mt-16 mb-6 flex w-full items-center justify-start gap-x-8 overflow-x-auto overflow-y-hidden border-b pt-4">
        {tabs.map((tab) => (
          <span
            key={tab}
          ...
          >
            ...
            {activeTab === tab && (
              <span className="bg-slate-900 absolute inset-x-0 -bottom-px h-[2px] w-full rounded-full" />
            )}
          </span>
        ))}
      </div>
