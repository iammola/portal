  ...
  return (
    <div
      className={classNames(
        "border-slate-200 relative flex w-full flex-row items-stretch justify-start gap-x-3 rounded-xl border bg-white ring-2",
        {
    ...
    >
      <div
        ...
        className="bg-slate-100 hover:bg-slate-200 focus:bg-slate-200 flex min-w-[65px] cursor-pointer flex-row items-center justify-center gap-x-0.5 rounded-l-xl px-3.5 py-3 focus:outline-none"
      >
        {countryFlag}
        {allowRegionChange && (
          <ChevronUpIcon
            className={classNames("fill-slate-600 h-6 w-6", {
      ...
      <div className="flex grow flex-row items-center justify-start gap-x-2 rounded-r-xl">
        <span className="text-slate-500 text-lg font-medium tracking-wide">+{countryCode}</span>
        <div className="flex h-full w-[12.5rem] grow items-center rounded-r-xl">
          <Input
            ...
            className="text-slate-600 h-[3.75rem] w-full grow rounded-r-xl !border-none !px-0 !py-3.5 text-lg font-semibold tracking-wide !ring-transparent focus:outline-none"
          />
        </div>
      </div>
      {allowRegionChange && (
